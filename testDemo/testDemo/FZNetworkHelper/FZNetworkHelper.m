    ...
    NSURLSessionDownloadTask *downloadTask=[session downloadTaskWithRequest:request completionHandler:^(NSURL * _Nullable location, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (!error) {
            if (location==nil) {
                ...
                NSURL *saveUrl;
                if (savePath) {
                    saveUrl=[NSURL fileURLWithPath:[NSString stringWithFormat:@"%@",savePath]];
                }else{
                    //当地址为nil时就用默认保存地址
                    saveUrl=[NSURL fileURLWithPath:[NSString stringWithFormat:@"%@%@",[PublicFunc GetSandBoxPathWithType:SandBoxPathLibrary],[response suggestedFilename]]];
                }
                NSError *fileOperateError;
                ...
                //保存到指定地址
                [fileManager copyItemAtURL:location toURL:saveUrl error:&fileOperateError];
                if (fileOperateError) {
                    NSLog(@"didFinishDownloadingToURL:Error is %@",fileOperateError.localizedDescription);
